...
<template>
  ...
  <div class="wrapper">
    ...
    <v-card
      class="headers !flex !flex-col !justify-center
...
<<<<<<< HEAD <script setup lang="ts">
  ...
  >>>>>>> 6ffadbf (profile page design, mobile view isnt tested yet)
  ...
  type RegisterSchemaType = zod.infer<typeof baseRegisterSchema>
    ...
    const regErrors = ref<zod.ZodFormattedError<RegisterSchemaType> | null>(null)
            ...
            <transition name="fade" enter-active-class="transition ease-out duration-300"
              ...
              <div v-if="isLoginForm">
                ...
                <form @submit.prevent="onLogin">
                  ...
                  <v-text-field name="pw" label="Jelszó" v-model="loginForm.password" variant="outlined"
              ...
              <div v-else>
                ...
                <form @submit.prevent="onRegistration">
                  ...
                  <v-text-field label="Felhasználónév" name="username" v-model="regForm.username" required
                    variant="outlined" density="comfortable" :error-messages="regErrors?.username?._errors[0]"
                    class="!mb-5"></v-text-field>

                  <v-text-field label="Jelszó" v-model="regForm.password" name="pw" required variant="outlined"
                    density="comfortable" :type="showPassword ? 'text' : 'password'"
                    @click:append-inner="togglePassword" class="relative !mb-5"
                    :error-messages="regErrors?.password?._errors[0]">
                    <button @click="togglePassword"
                      class="absolute right-2 top-1/2 transform -translate-y-1/2 text-white hover:text-gray-700 focus:outline-none"
                      type="button" :append-inner-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'">
                  ...
                  <v-text-field label="Jelszó megerősítés" v-model="regForm.confirmPassword"
                    ...
                    name="pw" required variant="outlined" density="comfortable"
      ...
      =======
      >>>>>>> 3d0494f (login/reg toast now gets api response message)
