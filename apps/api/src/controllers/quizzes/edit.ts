...
import db from "@/db";
import { insertQuizCardsSchema, insertQuizSchema, languagesTable, quizCardsTable, quizLanguagesTable, quizTagsTable, quizzesTable, tagsTable } from "@/db/schemas";
...
const editHandlers = GLOBALS.CONTROLLER_FACTORY(checkJwt(), zv('json', z.object({
...
})), zv('param', z.object({ quizId: z.string().uuid() })), async (c) => {
    const { quizId } = c.req.valid('param');
    const { userId } = c.get('accessTokenPayload');

    const { quiz: modifiedQuiz, cards: newCards, languageISOCodes: newLanguageISOCodes, tags: newTags } = c.req.valid('json');
    ...
    const quiz = await db.query.quizzesTable.findFirst({
        where: and(eq(quizzesTable.id, quizId), eq(quizzesTable.user_id, userId)),
        with: {
            cards: true,
            tags: {
                columns: {},
    ...
    if (modifiedQuiz) {
        let parsedNewQuiz;
        try {
            const rawFileBuf = Buffer.from(modifiedQuiz.banner.split(';base64,')[1], 'base64');
        ...
        Object.entries(parsedNewQuiz).reduce((acc, [key, value]) => {
            if (quiz[key as keyof typeof quiz] !== value) {
                if (value instanceof Buffer) {
                    if (value.toString('base64') === (quiz[key as keyof typeof quiz] as Buffer).toString('base64')) {
                        return acc;
                    }
                }
                // @ts-ignore impossible to type reduce idc
                acc[key] = value;
            }
            return acc
        }, quiz)
    ...
    try {
        await db.transaction(async (tr) => {
            await tr.update(quizzesTable).set({ ...quiz }).where(and(eq(quizzesTable.id, quizId), eq(quizzesTable.user_id, userId)));


            if (newCards) {
                await tr.delete(quizCardsTable).where(eq(quizCardsTable.quiz_id, quizId));
                newParsedCards.forEach(async (card) => {
                    await tr.insert(quizCardsTable).values({ quiz_id: quizId, ...card });
                })
            }
            ...
            if (newLanguageISOCodes) {
                for (const isoCode of newLanguageISOCodes) {
                    ...
                    await tr.insert(quizLanguagesTable).values({ language_id: languageId.id, quiz_id: quizId }).onConflictDoNothing();
                }
            };
            ...
            if (newTags) {
                for (const tag of newTags) {
                    ...
                    await tr.insert(quizTagsTable).values({ tag_id: tagId.id, quiz_id: quizId });
                }
            }
        });
    } catch (e) {
