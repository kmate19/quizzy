...
const forgotPasswordHandler = GLOBALS.CONTROLLER_FACTORY(zv('json', LoginUserSchema.omit({ password: true })), async (c) => {
    ...
    if (exist.length > 0) {
        await db.delete(userTokensTable).where(and(eq(userTokensTable.user_id, user.id), eq(userTokensTable.token_type, "forgot_password")));
    }
