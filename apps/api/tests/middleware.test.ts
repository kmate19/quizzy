...
describe("tests for middleware", () => {
    describe("jwt middleware", () => {
        ...
        test("it fails if user doesnt have the required role", async () => {
            ...
            const res = await client.auth.authed.$get({ query: { role: "admin" }, }
                , { headers: { cookie: cookies.join(';') } });

            const json = await res.json() as ApiResponse;
            expect(res.status).toBe(403);
            expect(json).toHaveProperty('error');
            expect(json?.error?.message).toBe("user does not have the required role");
        });
        test("it succeeds if user does have the required role", async () => {
            const { cookies } = await registerAndLogin(client);

            const [user] = await db.select().from(schema.usersTable).where(eq(schema.usersTable.username, "mateka"));
            const [adminRole] = await db.select().from(schema.rolesTable).where(eq(schema.rolesTable.name, "admin"));
            await db.insert(schema.userRolesTable).values({ user_id: user.id, role_id: adminRole.id }).onConflictDoNothing();
        ...
        test("it refreshes acccess token if expired", async () => {
            ...
            // expire the jwt
            const decoded = await verify(jwt, ENV.ACCESS_JWT_SECRET()) as QuizzyJWTPAYLOAD;

            decoded.exp = Math.floor(Date.now() / 1000) - 10;

            const newJwt = await sign(decoded, ENV.ACCESS_JWT_SECRET());

            const cookiesModified = [`${GLOBALS.ACCESS_COOKIE_NAME}=${newJwt};`
                .concat(cookies.join(';').split(';').slice(1).join(';'))];
        ...
        test("it fails if refresh token is expired", async () => {
            ...
            const [user] = await db.select().from(schema.usersTable).where(eq(schema.usersTable.username, "mateka"));
            const [userToken] = await db.select().from(schema.userTokensTable).where(and(
                eq(schema.userTokensTable.user_id, user.id), eq(schema.userTokensTable.token_type, "refresh")));
            ...
            await db.update(schema.userTokensTable).set({ expires_at: new Date() }).where(eq(schema.userTokensTable.id, userToken.id));
