...
describe("reserving session", () => {
    test("fails if time difference is more than 1500 ms", async () => {
        const res = await testClient(hono).ws.reserve.session[":code?"].$post({
            param: { code: "" },
            query: { ts: (Date.now() - 2000).toString() },
        });
    ...
    describe("if code is included in param", () => {
        test("fails if theres no session with that code", async () => {
            const res = await testClient(hono).ws.reserve.session[
                ":code?"
            ].$post({
                param: { code: "random123" },
                query: { ts: Date.now().toString() },
            });
            expect(res.status).toBe(400);
        });
        test("success if theres session with that code", async () => {
            const createRes = await testClient(hono).ws.reserve.session[
                ":code?"
            ].$post({
                param: { code: "" },
                query: { ts: Date.now().toString() },
            ...
            const res = await testClient(hono).ws.reserve.session[
                ":code?"
            ].$post({ param: { code }, query: { ts: Date.now().toString() } });
            expect(res.status).toBe(200);
